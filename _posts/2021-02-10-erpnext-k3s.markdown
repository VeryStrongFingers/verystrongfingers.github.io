...
Now your ERPNext instance is operational, and you have a site setup.

The final setup is to setup ingress route. We'll be using it to let Kubernetes knows the ERPNext site will route through the ingress-controller, which under the hood is the built-in Traefik LoadBalancer service in `kube-system`.
